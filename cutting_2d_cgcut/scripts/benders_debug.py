...
# candidate: single items + zero + top 30 by value
cand=set([(0,)*m])
for i in range(m):
...
cand.update(top)
cand=sorted(cand)
print("cand",len(cand),"maxval",max(val[p] for p in cand))
C=cand
n=len(C)
def solve_sub(ybar):
    ...
    con_item=[]
    for i in range(m):
        con_item.append(model.add_linear_constraint(sum(C[k][i]*x[k] for k in range(n))<=q[i]))
    ...
    dv=res.dual_values()
    pi=[dv[con_item[i]] for i in range(m)]
    mu=dv[con_sheet]
    gamma=[dv[con_ub[k]] for k in range(n)]
    return res.objective_value(), pi, mu, gamma, res.termination.reason
def solve_master(cuts):
    ...
    theta=model.add_variable(lb=0.0,ub=float("inf"),is_integer=False,name="theta")
    model.add_linear_constraint(sum(y[k] for k in range(n))<=1)
    # initial valid cut theta <= sum v_p y_p
    model.add_linear_constraint(theta <= sum(val[C[k]]*y[k] for k in range(n)))
    ...
    return yv, res.objective_value(), res.termination.reason
cuts=[]; best_LB=0; best_y=None
t0=time.time()
for it in range(100):
    ystar, ub, reason=solve_master(cuts)
    LB, pi, mu, gamma, sreason=solve_sub(ystar)
    if LB>best_LB:
        best_LB=LB; best_y=ystar
    print(f"it={it+1} masterUB={ub:.6f} subLB={LB:.6f} gap={ub-LB:.3e} ncuts={len(cuts)}")
    if ub - LB <= 1e-6 or it>=50:
        break
    cuts.append((sum(q[i]*pi[i] for i in range(m)), mu, gamma))
print("time",round(time.time()-t0,2),"bestLB",best_LB)
print("best pattern",[C[k] for k in range(n) if best_y and best_y[k]==1])
# final integer MIP over candidate
model=mathopt.Model(name="final")
z=[model.add_variable(lb=0.0,ub=1.0,is_integer=True,name=f"z{k}") for k in range(n)]
for i in range(m):
    model.add_linear_constraint(sum(C[k][i]*z[k] for k in range(n))<=q[i])
model.add_linear_constraint(sum(z[k] for k in range(n))<=1)
model.maximize(sum(val[C[k]]*z[k] for k in range(n)))
res=mathopt.solve(model, mathopt.SolverType.HIGHS, params=mathopt.SolveParameters(time_limit=datetime.timedelta(seconds=60),enable_output=False))
print("final IP",res.termination.reason,res.objective_value())
