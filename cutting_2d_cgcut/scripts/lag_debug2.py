...
def best_pattern(c):
    ...
    return F(W,H)
# exact LB via capped pool max (shortcut to 244)
@lru_cache(maxsize=None)
    ...
    for y in range(1,h):
        ...
        for p1 in P1:
            for p2 in P2:
                ...
                if all(t[j]<=q[j] for j in range(m)): res.add(t)
    return res
P=capped_patterns(W,H); LB=0
for p in P:
    val=sum(p[i]*v[i] for i in range(m))
    if val>LB: LB=val; bestp=p
print("LB",LB,"bestp",bestp)
pi=[0.0]*m
best_UB=1e18; best_pi=None
T=2000
for t in range(T):
    c=[v[i]-pi[i] for i in range(m)]
    val,counts=best_pattern(c)
    UB=sum(pi[i]*q[i] for i in range(m)) + val
    if UB<best_UB: best_UB=UB; best_pi=list(pi)
    g=[q[i]-counts[i] for i in range(m)]
    gnorm2=sum(gg*gg for gg in g)
    if gnorm2<1e-12:
        print("zero grad at",t); break
    # Polyak: minimize convex L -> pi = pi - step*g
    step=(UB-LB)/gnorm2
    step=max(0.0, min(step, 0.5))
    pi=[max(0.0, pi[i]-step*g[i]) for i in range(m)]
    if t%200==0 or t==T-1:
        print(f"t={t} UB={UB:.6f} bestUB={best_UB:.6f} step={step:.4f} counts={counts} pi={[round(x,3) for x in pi]}")
print("final best_UB",best_UB,"gap%",(best_UB-LB)/LB*100)
