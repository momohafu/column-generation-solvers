...
LB=(total+L-1)//L
print('m',m,'L',L,'total',total,'LB',LB,'demand',sum(d for _,d in items))
K=LB
...
dt=time.time()-t0
print('termination',res.termination.reason,'time',dt)
print('primal',res.primal_bound,'dual',res.dual_bound,'best_obj_bound',res.best_objective_bound)
if res.termination.reason in (mathopt.TerminationReason.OPTIMAL, mathopt.TerminationReason.FEASIBLE):
    yv=[round(res.variable_values(v)) for v in y]
    print('y',yv,'used',sum(yv))
    for k in range(K):
        if yv[k]:
            pat=[int(round(res.variable_values(x[i][k]))) for i in range(m)]
