...
def greedy_repair(selected):
    ...
    while not all(covered):
        ...
        for j in range(n):
            ...
            if best is None or ratio<best[0]: best=(ratio,j,len(new))
        j=best[1]; sel.add(j)
        for i in colrows[j]: covered[i]=True
    return list(sel)
sel0=greedy_repair([]); UB=sum(costs[j] for j in sel0)
pi=[0.0]*m; lam=2.0; best_L=-1e9; best_pi=None; best_x=None; no_impr=0; max_iter=1000
...
for it in range(max_iter):
    ...
    if no_impr>=100: lam/=2.0; no_impr=0
selb=greedy_repair([j for j in range(n) if best_x[j]]); cb=sum(costs[j] for j in selb)
if cb<UB: UB=cb
